...
Human & Same-window human baseline & 31,443 & 22,538 & 95,424 & 18,559 & 21,401 \\
\midrule
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{repository-background-reference-style-13pt.png}
...
\subsection{Analysis subsets}
\label{sec:analysis_subsets}

After deduplication, the scoped corpus contains 325,523 agent-authored PRs, 29,808 human-authored PRs, and 1,206 bot-authored PRs. These groups form the common starting point for the analysis. 
All downstream constructs are derived from reconstructed public PR timelines, but the unit of analysis differs by research question.
RQ1 and RQ4 use PR-level denominators for entry and integration outcomes.
RQ2 uses feedback-event and feedback-body denominators.
RQ3 uses both PR-level pathway denominators and cycle-level denominators for latency and response measures.
We state the exact denominators where each result appears.
This design avoids comparing event-level percentages with PR-level percentages and keeps PRs without feedback in pathway and endpoint denominators while excluding them only from measures that require feedback, response, or endpoint timestamps.
\subsection{PR Timeline Construction}
\label{sec:method_timeline}

\runinhead{PR Timeline and Action Types}
For each PR, we reconstruct an ordered public timeline by merging \texttt{Open}, \texttt{Comment}, \texttt{Review}, and \texttt{Commit} actions with normalized actor logins and UTC timestamps.
The first \texttt{Open} action defines the opening time and normalized PR-author login. 
The resulting timeline is the ordered action sequence used through the paper.

The first entry event is defined as the earliest \texttt{Comment} or \texttt{Review} action from a non-author whose timestamp is strictly later than the opening time.
We use \emph{entry} for this first PR-thread contact so that feedback comments and reviews are not conflated with later code responses.
Commits are handled separately as response-eligible code activity in RQ3.
Actions authored by the PR author are excluded so that self-follow-up does not count as an entry.
When multiple candidate actions share the same timestamp, ties are broken in the order \texttt{Comment} to \texttt{Review}.
This rule yields a deterministic first-entry label when multiple conversational actions occur at the same recorded time.

The same rule is applied consistently to agent-, human-, and bot-authored PRs in the baseline comparison. 
PRs without candidate entry events are excluded from first-entry actor measures but kept in the full PR corpus for pathway analyses.

The lower panel of Figure~\ref{fig:method_rq_workflow_timeline} illustrates the PR-timeline representation used throughout the paper and shows the reconstruction procedure on a representative PR.

\subsection{Baseline construction}
\label{sec:baseline_construction}

Baseline sets are constructed from the same study window as the agent PR corpus (2025-04-01 to 2026-01-31) and processed with the same canonicalization pipeline. For all author groups, each PR is mapped to a canonical key of the form \texttt{repo\_full\_name} \texttt{pr\_number}. When GraphQL pull-request identifiers are available, they are first aligned to this canonical key so that PR rows and PR-action rows can be joined consistently across source tables. After time-window filtering, duplicate PR rows are removed by canonical PR key, keeping the earliest open action for each PR.

The human baseline is drawn from the companion human PR corpus and is used only for cross-group comparison. The bot baseline is defined coarsely from normalized author logins that match common bot patterns, including explicit \texttt{[bot]} markers and dependency-update accounts such as Dependabot and Renovate. In this design, bot-authored PRs are grouped into a single bot baseline for cross-group comparison.

The three author groups hence share the same study window, PR-key construction, date filtering, and deduplication pipeline.
The cross-group analyses support the same-window comparison.

\subsection{Human-Agent PR Workflow Model}
\label{sec:method_workflow_model}

Prior work studies modern code review, PR mining, automation in PR threads, and agent-authored PRs through signals such as reviews, code changes, latency, and merge outcomes.
We connect these signals in one PR trajectory and use it to organize the four RQs and the measures below.

We model each PR as a Human-Agent PR workflow with four stages.
They are \emph{Entry}, \emph{Feedback}, \emph{Feedback-response Cycle}, and \emph{Integration}.
The stages are grounded in GitHub PR events. 
Entry begins with the PR \texttt{Open} event and tracks whether a later non-author PR-thread entry appears and who first triages the PR.
% ORIGINAL: Feedback uses PR \texttt{Comment} and \texttt{Review} events from humans, agents, or automation bots, and is characterized by actor, body form, and semantic intent.
Feedback uses PR \texttt{Comment} and \texttt{Review} events from humans, agents, or automation bots, and is characterized by actor, body form, and feedback intent.
Feedback-response Cycle links feedback blocks to later response-eligible \texttt{Commit} events, measuring response timing, response actors, and code-change magnitude. 
Integration uses merge and close metadata to classify the PR as merged, closed without merge, or still open at data collection.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_method_rq_workflow_timeline.pdf}
  \caption{Human-agent PR workflow model (upper) and example PR timeline for GitHub PR microsoft/testfx/pull/5633 (lower). Bot labels are automation bots.}
  \Description{Two-part figure.
  The upper panel shows an agent-authored pull request moving from open and entry through feedback and response states to an endpoint, with RQ1 through RQ4 mapped to the stages.
  The lower panel shows an example pull request timeline with open, commits, reviews, comments, response commits, and closure.}
  \label{fig:method_rq_workflow_timeline}
\end{figure*}

Figure~\ref{fig:method_rq_workflow_timeline} maps the four RQs onto this model, distinguishing PRs that share an endpoint but differ in triage, feedback, response, and revision.

\subsection{Taxonomy Construction}
\label{sec:method_taxonomy_construction}

\subsubsection{RQ1: Entry}
\label{sec:method_rq1_entry}

\runinhead{Interaction Sequence}
To characterize how collaboration begins, we extract entry events from PR timelines.
For each PR, raw entry is the earliest non-author PR-thread comment or review after opening.
Retained entry applies the feedback filter described below before selecting the first retained event.
Commits are not treated as entry feedback and are reserved for code-response analyses.

\runinhead{Entry Measures}
The analysis reports raw and retained entry rates and first retained entry actor.

\runinhead{Engaging Developers and Coverage}
An engaging developer is defined as any account that performs at least one human PR action in an agent-authored PR during the study window, for example, commenting, reviewing, committing, or creating an issue. 
Accounts identified as non-human automation (bots) or coding agents are excluded using the tracking signals in Table~\ref{tab:dataset_overview}. 
The packaged user table contains 58,262 accounts with account-creation timestamps and contribution summaries of the past year preceding data collection, of which 56,190 have nonzero public contribution volume.
For first-human-triager footprint analyses, we join PR-thread accounts to this public-profile table and report unmatched accounts as no-metadata.

\runinhead{Public GitHub Footprint Score}
Following large-scale GitHub profiling work~\cite{baoLargeScaleStudy2021}, we construct an empirical public GitHub footprint score for matched first human responders from two public profile signals, account tenure and cumulative public contribution volume.
For each user $u$, $\mathrm{tenure}(u)$ is the number of days between \texttt{created\_at} and a fixed reference time, and $\mathrm{contrib}(u)=\#\mathrm{commits}(u)+\#\mathrm{PRs}(u)+\#\mathrm{reviews}(u)+\#\mathrm{issues}(u)$.
We apply $\log(1+x)$ to both components, winsorize each transformed component at the 5th and 95th percentiles, and min--max normalize to $[0,1]$.
The main specification is an equal-weight score $\mathrm{footprint}(u)=0.5\,\widehat{\mathrm{tenure}}(u)+0.5\,\widehat{\mathrm{contrib}}(u)$.

\runinhead{Footprint Strata}
We convert the continuous score into empirical quartiles \textbf{Q1-4 footprint} within the covered user subset.
For footprint-stratified PR-level analyses, each PR receives the stratum of its first matched non-author human responder.
PRs without a matched responder remain unlabeled and are excluded from these summaries.
...
\begin{figure}[tbp]
  ...
  \caption{Human footprint score quartiles and scoreable first-human triagers.}
...
RQ2 characterizes the feedback that appears after an agent-authored PR enters the PR thread.
Feedback events consist of non-author PR-thread comments and reviews after PR opening.
Each event retains its event type, actor, timestamp, and available text body.
We report comments and reviews separately where useful because short comments, formal review bodies, and generated review reports can carry different kinds of feedback.

\runinhead{Feedback actors}
Each feedback event receives the refined actor class defined above.
Accounts marked by the coarse detector but not safely separable by the refined rules remain \emph{unresolved agent/bot}.
Missing or unrecognized actors remain \emph{unknown}.
Unlike RQ1, which focuses on the first retained triager per PR, RQ2 summarizes all eligible feedback events.
When the feedback actor is a matched human, we attach the public-footprint stratum defined in RQ1.

\runinhead{Syntax features}
% ORIGINAL: Following work on code-review usefulness, review-comment classification, and review automation~\cite{rahmanPredictingUsefulnessCode2017,liAutomaticClassificationReview2017,goldmanWhatTypesCode2025,wesselDontDisturbMe2021,wesselEffectsAdoptingCode2020,wesselQualityGatekeepersInvestigating2022}, we extract deterministic body-form features from each available feedback body before interpreting semantic intent.
Following work on code-review usefulness, review-comment classification, and review automation~\cite{rahmanPredictingUsefulnessCode2017,liAutomaticClassificationReview2017,goldmanWhatTypesCode2025,wesselDontDisturbMe2021,wesselEffectsAdoptingCode2020,wesselQualityGatekeepersInvestigating2022}, we extract deterministic body-form features from each available feedback body before assigning intent labels.
These features capture body length, code grounding through fenced or inline code markers, structured packaging through Markdown bullets, numbered lists, tables, or summary-like sections, and boilerplate/status markers for CI, deployment, routing, helper, or administrative messages.

% ORIGINAL: \runinhead{Intent taxonomy}
\runinhead{Intent taxonomy}
% ORIGINAL: We construct a semantic intent taxonomy for PR-thread feedback following the human-in-the-loop LLM taxonomy workflow~\cite{shahUsingLargeLanguage2023}, which uses an initial taxonomy, LLM-assisted labeling, human validation, and full-corpus application.
We construct an intent taxonomy for PR-thread feedback following the human-in-the-loop LLM taxonomy workflow~\cite{shahUsingLargeLanguage2023}, which uses an initial taxonomy, LLM-assisted labeling, human validation, and full-corpus application.
Our initial taxonomy is seeded from prior work on modern code review and review-comment classification.
Code review is known to support defect finding, maintainability discussion, knowledge transfer, and integration decisions~\cite{bacchelliExpectationsOutcomesChallenges2013,sadowskiModernCodeReview2018,mcintoshEmpiricalStudyImpact2016}.
PR reviews and comments have also been classified by correctness, management, interaction, and other review functions~\cite{liAutomaticClassificationReview2017}.
Recent work on useful or LLM-generated code-review comments motivates distinguishing actionable concerns from non-actionable or generated feedback~\cite{rahmanPredictingUsefulnessCode2017,goldmanWhatTypesCode2025}.
% ORIGINAL:
% Based on these sources and pilot inspection of our feedback bodies, we define seven primary intent labels used throughout the text and figures.
Based on these sources and pilot inspection of our feedback bodies, we define seven primary intent labels for aggregate feedback summaries.
They are \textit{Functional}, \textit{Maintainability}, \textit{Documentation}, \textit{Discussion}, \textit{Process}, \textit{Validation}, and \textit{Non-actionable}.
\textit{Maintainability} covers refactoring and cleanup requests, \textit{Process} covers integration, CI, merge, and workflow coordination, and \textit{Validation} covers evidence, reproduction, test, or verification requests.
Figures abbreviate labels only for space, such as \textit{Maint.}, \textit{Docs}, and \textit{Non-action}.

\runinhead{Intent labeling}
We use the codebook for LLM-assisted labeling.
Two human annotators independently labeled 141 randomly sampled feedback events using the same primary-label taxonomy.
% ORIGINAL: After applying the edited disagreement table, 104 events had matching primary intent labels, yielding Cohen's $\kappa=0.683$, which indicates substantial agreement~\cite{cohenCoefficientAgreementNominal1960}.
After applying the edited disagreement table, 104 events had matching primary intent labels, yielding Cohen's $\kappa=0.683$, which indicates substantial agreement~\cite{cohenCoefficientAgreementNominal1960}.
The remaining 37 disagreements are excluded from the consensus-gold evaluation.
% ORIGINAL:
% For the full-corpus labeling, we use the selected scalable setup of \texttt{openai/gpt-4o-mini} with batch size 20 and two examples per label.
% On 29 held-out consensus rows, this setup achieves 0.690 accuracy, 0.577 macro-F1, 0.676 weighted-F1, and produces no invalid outputs.
% We apply this setup to the remaining eligible feedback events.
For the full-corpus labeling, we use the selected scalable setup of \texttt{openai/gpt-4o-mini} with batch size 20 and two examples per label.
% ORIGINAL: On 29 held-out consensus rows, this setup achieves 0.690 accuracy, 0.577 macro-F1, 0.676 weighted-F1, and produces no invalid outputs.
For this primary-intent labeling task, the 29-row held-out consensus set yields 0.690 accuracy, 0.577 macro-F1, 0.676 weighted-F1, and no invalid outputs.
% ORIGINAL:
% Because this held-out set is small, intent labels are used as descriptive strata rather than as standalone evidence for causal or quality claims.
% ORIGINAL: Intent labels are used as an aggregate descriptive layer: they summarize what retained feedback appears to ask for, but they do not define entry, cycle, pathway, revision, or integration measures.
These labels are used only as an aggregate descriptive layer: they summarize what retained feedback appears to ask for, but they do not define entry, cycle, pathway, revision, or integration measures.
The primary workflow claims rely on event type, actor class, timestamps, commits, and endpoint metadata.
% ORIGINAL: Given the small held-out set, intent-stratified contrasts should be read as descriptive patterns rather than standalone evidence about feedback quality or causal effects.
Given the small held-out set, intent-stratified contrasts should be read as descriptive patterns rather than standalone evidence about feedback quality or causal effects.
We apply this setup to the remaining eligible feedback events.

\subsubsection{RQ3: Feedback-Response Cycles}
\label{sec:method_rq3_cycles}

RQ3 asks whether commits follow feedback, how many cycles occur, how long they take, and how much code changes.

\runinhead{Blocks and cycles}
A \emph{feedback block} is a contiguous run of feedback events.
A new block begins after an intervening commit. 
A \emph{response block} is the set of commits after a feedback block and before the next feedback block. 
A \emph{feedback-response cycle} exists when a feedback block has at least one following commit in its response block.
If no later commit shows, the block is feedback without a code response.

Response-eligible commits occur after the PR is opened and before the PR is merged or closed. 
We exclude routine merge artifacts, merge-button or merge-queue boundary commits, and commits at or after terminal merge/close. 
Events are sorted by timestamp, event priority, and event ID. 
At identical timestamps, feedback is ordered before commits, so same-time feedback followed by a commit can be counted as a zero-latency response.

\runinhead{Pathway taxonomy}
Each scoped PR receives one feedback-response pathway. 
The pathway taxonomy is derived exclusively from the reconstructed PR timeline dataset. 
The rules are mutually exclusive and exhaustive for scoped PR timelines.
Each PR has no visible feedback, feedback without a later code response, one response cycle, multiple response cycles, or terminal feedback after the final code change.

\begin{itemize}[leftmargin=1.3em, itemsep=0.15em, topsep=0.2em]
\item \textit{No visible feedback}. No feedback block appears.
\item \textit{Feedback without code response}. Feedback exists, but no later response-eligible commit follows.
\item \textit{Single-feedback response cycle}. Exactly one feedback block is followed by a response block.
\item \textit{Multi-feedback response cycle}. Two or more feedback blocks are followed by response blocks.
\item \textit{Late feedback after final code change}. The final feedback block starts after the final response block.
\end{itemize}

The late-feedback category takes precedence over the single- or multi-cycle count because it describes the PR's final state.
A PR in this category may yet have earlier cycles.

\runinhead{Cycle timing and code response}
Cycle latency is the time from the end of a feedback block to the first commit in its response block. 
Latency summaries use only cycles with a response commit. 
Cycle times are skewed, so we report medians, IQRs, and upper percentiles.

We measure code-response size at two levels. 
At the cycle level, we count response commits, changed lines, changed files, and changed lines per feedback event for each response block. 
At the PR level, we split response-eligible commits into pre-feedback and post-feedback commits using the first feedback event.
PRs without visible feedback, or without later response commits, remain in the PR-level denominator with zero post-feedback code response.

We measure changed lines as additions plus deletions when both are available.
Changed files come directly from GitHub's per-commit \texttt{changed\_files}.
When a stat is missing, we leave it missing and report how often it occurs.

\runinhead{Response actors}
For response blocks, we retain the committer GitHub accounts and other activities following feedback.

\subsubsection{RQ4: Integration}
\label{sec:method_taxonomy_rq4}

% ORIGINAL: RQ4 asks how the PR workflows characterized in the earlier stages are associated with repository endpoints.
% ORIGINAL: Each reconstructed PR is assigned to one of the endpoint categories below.
% ORIGINAL: A PR is classified as \emph{merged} when a merge timestamp is present.
% ORIGINAL: A PR is classified as \emph{closed without merge} when a close timestamp is present but no merge timestamp is present.
% ORIGINAL: A PR with no close timestamp by the end of data collection is classified as \emph{open}.
% ORIGINAL: When merge and close metadata are both present, we consider it merged, as merged PRs are also closed by GitHub workflow.
RQ4 asks how the PR workflows characterized in the earlier stages are associated with repository endpoints.
Each PR receives one endpoint label: \emph{merged} when a merge timestamp is present, \emph{closed without merge} when a close timestamp appears without a merge timestamp, and \emph{open} when no close timestamp appears by the end of data collection.
When merge and close metadata are both present, we classify the PR as merged because merged PRs are also closed by GitHub workflow.

% ORIGINAL: RQ4 keeps the full PR-level denominator so that PRs without visible feedback still receive endpoint labels.
% ORIGINAL: The primary stratification is the RQ3 feedback-response pathway, which summarizes whether feedback appeared, whether code activity followed, and whether the final feedback was close- or merge-adjacent.
% ORIGINAL: Feedback-to-endpoint timing is computed only when the first feedback strictly precedes the merge/close timestamp.
% ORIGINAL: Other PRs remain in endpoint denominators but are excluded from latency summaries.
% ORIGINAL: Open or without-close PRs are right-censored by the data-collection window and are interpreted as extraction-time states rather than final states.
% ORIGINAL: Endpoint contrasts use percentage point (pp) differences from the specified baseline.
% ORIGINAL: For construct-specific panels, the baseline is the endpoint rate over the construct's covered PR subset before splitting by that construct.
RQ4 keeps the full PR-level denominator, so PRs without visible feedback still receive endpoint labels.
The primary stratification is the RQ3 feedback-response pathway, which summarizes whether feedback appeared, whether code activity followed, and whether final feedback was close- or merge-adjacent.
Feedback-to-endpoint timing is computed only when first feedback strictly precedes merge or close; other PRs remain in endpoint denominators but are excluded from latency summaries.
Open or without-close PRs are right-censored by the data-collection window and are interpreted as extraction-time states rather than final states.
Endpoint contrasts use percentage point (pp) differences from the specified baseline.
For construct-specific panels, the baseline is the endpoint rate over the construct's covered PR subset before splitting by that construct.
% ORIGINAL:
% The covered subsets are first-retained-actor PRs for actor rows, first-retained-intent or body-form PRs for intent/body rows, and scoreable first-human-triager PRs for footprint rows.
% ORIGINAL: The covered subsets are first-retained-actor PRs for actor rows, first-retained descriptive intent or body-form PRs for intent/body rows, and scoreable first-human-triager PRs for footprint rows.
The covered subsets are first-retained-actor PRs for actor rows, first-retained-intent or body-form PRs for intent/body rows, and scoreable first-human-triager PRs for footprint rows.
Reported deltas are stratified endpoint rates minus the construct baseline.
No regression adjustment is implied.
